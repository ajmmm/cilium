...
| bpf.lbUnsupportedProtocolAction | string | `"forward"` | Configure how the datapath should handle unsupported protocols (forward, drop). This can also be configured on a per-service basis through the network.cilium.io/unsupported-protocol-action annotation. |
