	...
	if (is_ipv4)
		if (is_host) {
			if (l4->check != bpf_htons(0x118e))
				test_fatal("L4 checksum is invalid: %x != %x", l4->check, bpf_htons(0x118e));
		} else {
		...
		if (is_host) {
			if (l4->check != bpf_htons(0x3f82))
				test_fatal("L4 checksum is invalid: %x != %x", l4->check, bpf_htons(0x3f82));
		} else {
			if (l4->check != bpf_htons(0x3f84))
				test_fatal("L4 checksum is invalid: %x != %x", l4->check, bpf_htons(0x843f));
		}
